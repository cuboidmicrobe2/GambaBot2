...
final class RockPaperScissors extends GameInstance {
    ...
    public bool $started = false {
        ...
        set(bool $sate) {
            if(!$this->started) $this->started = $sate;
        }
    }
    ...
    public function __destruct() {
        if($this->p1Points == $this->p2Points AND $this->started) {
            ...
            elseif($this->roundData[$this->round][$this->p2Uid] !== null AND $this->roundData[$this->round][$this->p1Uid] === null) {
        ...
        elseif($this->p1Points > $this->p2Points) {
            $this->p1Inv->setCoins($this->p1Inv->getCoins() + ($this->bet * 2));
            return;
        }
        elseif($this->p1Points < $this->p2Points) {
    ...
    public function makeMove(string $uid, RpsMove $move) : bool {
        if($uid != $this->p1Uid AND $uid != $this->p2Uid) throw new InvalidArgumentException($uid . ' is not a player in this game'); 
        
        $this->renew();

        if($this->roundData[$this->round][$uid] !== null) return false;
    ...
    public function movesDone() : bool {
        if($this->roundData[$this->round][$this->p1Uid] instanceof RpsMove AND $this->roundData[$this->round][$this->p2Uid] instanceof RpsMove) return true;
        return false;
    }
    ...
    public function checkWinner() : ?string {
        if($this->p1Points >= self::WIN_CON) return $this->p1Uid;
        elseif($this->p2Points >= self::WIN_CON) return $this->p2Uid;
        return null;
    }
    ...
    private static function calcWin(RpsMove $p1Move, RpsMove $p2Move) : string {
        ...
        for($i = 0; $i < count($moveLogic); $i++) {
            if($moveLogic[$i] != $p1Move) continue;

            if($p2Move == $moveLogic[$i + 1]) return 'p2';
            return 'p1';
        }
