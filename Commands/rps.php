...
$discord->listenCommand('rps', function(Interaction $interaction) use ($discord, $gamba) {
        ...
        if($game->makeMove($player, $move)) {
            ...
            if($game->movesDone()) {
                ...
                if($winner = $game->checkWinner()) {
                    ...
                    $winnerUsername = '$name';
                    $discord->users->fetch($winner)->then(function(User $user) use (&$winnerUsername) { $winnerUsername = getUsername($user); });

                    $scoreFormatted = '$score';
                    if($game->p1Points > $game->p2Points) $scoreFormatted = code($game->p1Points . ' - ' . $game->p2Points);
                    else $scoreFormatted = code($game->p2Points . ' - ' . $game->p1Points);
    ...
    $p1Name = '$name';
    $p2Name = '$name';
    $discord->users->fetch($p1)->then(function(User $user) use (&$p1Name) { $p1Name = getUsername($user); });
    $discord->users->fetch($p2)->then(function(User $user) use (&$p2Name) { $p2Name = getUsername($user); });
